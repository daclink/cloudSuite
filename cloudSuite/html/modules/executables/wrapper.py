...
executable ="path/to/processor.exe"
...
mod_data = check_output(executable ,shell=True)
...
filename = mod_id + "."
filename = filename + sys.argv[0].split(".")[0]
data     = filename + ".txt"
...
desc = desc + sys.argv[0].split(".")[0]

#mod_name = mod_id + "." + sys.argv[0].split(".")[0] + ".xml"
mod_name = sys.argv[0].split(".")[0]
...
methodName  = SubElement(module, "methodName").text = sys.argv[0]
